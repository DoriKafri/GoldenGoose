...
def _clear_truncated_transcript_caches():
    """No-op. Previously cleared all transcript/takeaways/DOPI caches,
    but that was destroying valid cached data. Now a no-op — bad cached
    data (if any) will be replaced naturally when videos are re-viewed.
    The Gemini hallucination fallback has been removed, so new entries
    cannot contain hallucinated transcripts."""
    logger.info("Cache clear migration is now a no-op — caches preserved.")
    return
